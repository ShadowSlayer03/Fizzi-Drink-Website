...
const Crosshair: React.FC<CrosshairProps> = ({ color = 'white', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    const attachHoverEffects = () => {
      // Get both links AND falling sprites
      const links: NodeListOf<HTMLAnchorElement> = containerRef?.current
        ? containerRef.current.querySelectorAll('a')
        : document.querySelectorAll('a');

      const sprites: NodeListOf<HTMLDivElement> = containerRef?.current
        ? containerRef.current.querySelectorAll('#falling-sprite')
        : document.querySelectorAll('#falling-sprite');
      ...
      sprites.forEach(sprite => {
        sprite.removeEventListener('mouseenter', enter);
        sprite.removeEventListener('mouseleave', leave);
        sprite.addEventListener('mouseenter', enter);
        sprite.addEventListener('mouseleave', leave);
      });
    };

    attachHoverEffects();

    // Watch for new sprites being added to DOM
